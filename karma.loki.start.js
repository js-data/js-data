function Index(definition) {
  this.idAttribute = definition.idAttribute;
  this.data = new loki.Collection('data', { indices: [this.idAttribute]});
  this.data.ensureUniqueIndex(this.idAttribute);
}

Index.prototype.get = function (key) {
  var item = this.data.by(this.idAttribute, key);
  return item ? item.value : item;
};

Index.prototype.insert = function (key, value) {
  var item = { value: value };
  item[this.idAttribute] = key;
  this.data.insert(item);
  return item.value;
};

Index.prototype.update = function (key, field, value) {
  var item = this.data.by(this.idAttribute, key);
  if (item) {
    JSData.DSUtils.set(item.value, field, value);
  }
  return item;
};
