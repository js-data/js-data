  ...
  it('should skip', () => {
    ...
    TYPES_EXCEPT_NUMBER.forEach(value => {
      assert.throws(
        ...
        Error,
        `[Query#skip:num] expected: number, found: ${typeof value}\nhttp://www.js-data.io/v3.0/docs/errors#400`
      )
    })
    assert.deepEqual(collection.skip(1), [{ id: 2 }, { id: 3 }], 'should have skipped 1')
    assert.deepEqual(collection.skip(2), [{ id: 3 }], 'should have skipped 2')
