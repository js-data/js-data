...
describe('DataStore collection methods', () => {
  ...
  it('should proxy Collection Methods', () => {
    ...
    assert((collection.createIndex as any).calledWithMatch('statusAndRole', ['status', 'role']),
    ...
    assert((collection.between as any).calledWithMatch([18], [30], { index: 'age' }),
      '[Collection.between] called with wrong arguments')

    sinon.replace(collection, 'toJSON', sinon.fake())
    store.toJSON('user', { id: 1 })
    assert((collection.toJSON as any).calledWithMatch({ id: 1 }),
