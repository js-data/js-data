...
describe('utils.findIndex', () => {
  ...
  it('can find the last index based on given function', () => {
    ...
    assert.equal(
      1,
      utils.findIndex(users, user => user.age === 25)
    )
    assert.equal(
      2,
      utils.findIndex(users, user => user.age > 19)
    )
    assert.equal(
      2,
      utils.findIndex(users, user => !user.spy)
    )
    assert.equal(
      0,
      utils.findIndex(users, user => user.name === 'John')
    )
    assert.equal(
      -1,
      utils.findIndex(users, user => user.name === 'Jimmy')
    )
    assert.equal(
      -1,
      utils.findIndex(null, user => user.name === 'Jimmy')
    )
  })
})
